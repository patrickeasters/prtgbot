...
def pause_device(device_id, paused):
    ...
    # make the API call
    resp = api_call('/api/pause.htm', {'id':device_id, 'pausemsg':'paused by prtgbot', 'action':pause_action})            
    if(resp):
    	return True
    resp.close()
...
@respond_to('^\s*pause (.*)', re.IGNORECASE)
